...
function hndLoad(_event: Event): void {
    ...
    let person: Person = new Person(_x, _y, _velX, _velY);
    persons.push(person);
    }
    function createBird(_x: number, _y: number, _velX: number, _velY: number): void {
    let bird: Bird = new Bird(_x, _y, _velX, _velY);
    birds.push(bird);
    }
    function createCloud(_x: number, _y: number, _velX: number): void {
    let cloud: Cloud = new Cloud(_x, _y, _velX);
    clouds.push(cloud);
    }
}
}